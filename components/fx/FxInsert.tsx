...
import { InsertFx } from "../../__generated__/InsertFx";
import { ListCurrencies } from "../../__generated__/ListCurrencies";

const INSERT_FX = gql`
mutation InsertFx($amount: Int, $date: date, $price: numeric, $currency_pair_id: Int) {
...
const FxInsert = () => {
    const [insertFx] = useMutation<InsertFx>(INSERT_FX);
    return (
...
const FxCurrencyDropdown = () => {
    const {loading, error, data} = useQuery<ListCurrencies>(LIST_CURRENCIES);
    ...
    >
        {data!.currency_pair.map(row => <Select.Option key={row.id} value={row.id}>
